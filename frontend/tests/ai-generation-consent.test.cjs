...
async function setup(data,writeOk=true){
 ...
 const Consent=load('AiConsent.tsx',mocks,async(url,options)=>{requests.push(options?.method??'GET');if(options?.method==='PUT'&&writeOk){const body=JSON.parse(options.body);data={...data,consents:[{provider:body.provider,notice_version:body.version,granted:body.granted}]};}return {ok:options?.method!=='PUT'||writeOk,json:async()=>data};});
 const Dialog=load('AiGenerationConsent.tsx',{...mocks,'./AiConsent':Consent});let view;
...
test('disabled notice, stale consent, failed write and withdrawal never authorize generation',async()=>{
 ...
 [{notices:[notice],consents:[{provider:'gemini',notice_version:'v2',granted:true}]},true,false],
